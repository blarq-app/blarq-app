...
- **Ronda 34 — Limpieza de conciliaciones mal asignadas + criterio conservador (sesión con MJ)**:
  ...
  - **Implementado en código (cierra #2 de ronda 32 "import toma el primero")**: el importador de cartolas (`/api/banco/import`) tenía una copia inline del auto-match con el bug "toma la primera del mismo monto" — se eliminó y ahora delega en la función compartida `tryAutoMatchMovementWithInvoices`. La decisión de match se extrajo a la función pura `decideMovementInvoiceMatch` (exige RUT directo o vía alias; no usa fecha) con test de regresión `scripts/test-conciliacion.ts` (7 casos, pasan). NO se automatizó el match por comercio (compras tarjeta) ni el desempate por fecha — quedan manuales a propósito.
  - **Sigue pendiente**: el sync SII (factura→mov, `tryAutoMatchInvoiceWithExistingMovs`) ya era conservador (RUT + 1 candidato), no se tocó. El desempate por fecha entre candidatas (#1 de ronda 32) se decidió NO implementar.
