...
- **Migración de la viva.** SQL aditivo escrito a mano en `scripts/sql/capitulos-obra-viva.sql` (NO usar `prisma migrate diff`: mezcla cambios de otras ramas, hoy incluye un `DROP COLUMN "revisado"` ajeno) + relleno con `scripts/migrar-capitulos-obra.ts` (dry-run y `--apply`). Crea 137 capítulos sobre 928 partidas sin mover ninguna.
