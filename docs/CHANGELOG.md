...
- **Pendiente 186**: en Casa Los Algarrobos V4, 30 líneas decían "no sigue al catálogo" y solo 5 eran decisión de MJ. Ahora aplicar el precio de la tienda web **no despega** la línea (y si estaba despegada, la reconecta). "Comparar con mi catálogo", al bajar el precio, **vuelve a conectarla**; antes no había forma de volver. Tipear un precio sigue despegando. Decisión en [ADR 2026-09-25](decisions/2026-09-25-artefactos-solo-mj-despega-la-linea.md).
- **El descuento de MJ, protegido de punta a punta**: en "Comparar con la tienda web" viene sin marcar y con aviso "Este descuento lo pusiste vos". Duplicar una versión y "Volver a lo enviado" ya no le borran la marca (los 10% de Teka la perdieron al crear la V4). Aplicar solo la foto tampoco. El editor la muestra en negrita apenas se guarda.
...
- Regresión: `scripts/test-186-despegue.ts` (35 casos). Corrección de datos de una vez: `scripts/aplicar-186-reconectar.ts` (solo marcas, $0 de movimiento).
...
- **Categoría Tiradores y link + Extraer al crear un herraje desde la partida (PR #439)**: para los pomos de Ducasse (PrestaShop, el extractor lo entiende). Ducasse permitido en el proxy de imágenes.
...
- **La barra del proyecto rompía 6 pantallas de una.** El header sticky de `/proyectos/[id]` usaba `-mx-6` para llegar de borde a borde, pero el `<main>` tiene `p-4` en el celular: 24 contra 16, o sea que la banda sobresalía 8px por lado y arrastraba Resumen, Presupuesto, EPs, Facturas y Lista de compra. Ahora los márgenes negativos valen exactamente el padding en cada breakpoint (`-mx-4 md:-mx-8`). En desktop además dejaba 8px de aire que tampoco correspondía.
- **Los otros desbordes eran barras de controles en una sola fila**: el buscador del catálogo de partidas (163px), los cinco botones del de materiales (251px), la toolbar de la lista de compra (405px), las solapas de facturas del proyecto (27px) y el rango de fechas de sus filtros (un bloque que no podía partirse). Todos apilan o scrollean solos abajo de su breakpoint.
- **Siete listas pasan a tarjetas en el celular, con la tabla intacta en el escritorio**: Facturas (la tabla mide 800px de mínimo), Movimientos del banco (840px), Facturas del proyecto (980px), Reglas por proveedor (830px), Reglas del banco, Gastos de contabilidad (720px) y Lista de compra. En todas el criterio es el mismo: arriba lo que identifica la fila y el monto, abajo lo que se edita desde el teléfono. `ProjectsTable` se arregló una vez y sirve para Dashboard, Proyectos y Cotizaciones.
- **La celda "Respaldo" del banco se extrajo a `RespaldoContenido`** en vez de duplicarla: la usan la tabla y la tarjeta, así que un arreglo futuro no puede quedar aplicado en un solo lado.
- **Tablas que eran `w-full` DENTRO de un `overflow-x-auto` nunca scrolleaban** — se achicaban hasta que los encabezados se pisaban ("REAL" encima de "DIFERENCIA" en Presupuesto vs Real). Se les puso ancho mínimo real en `resumen`, `CuadroResumenAvance`, `CostoDirectoDetalle` y el catálogo de materiales.
- **Los filtros dejan de tapar el contenido**: en Facturas los 9 filtros quedan detrás de un botón "Filtros (n)" con la búsqueda libre siempre visible; en Banco cada fila de filtros es una sola línea que scrollea y las tarjetas de totales van en fila sin el desglose ingresos/egresos. Antes había que scrollear una pantalla entera antes de ver el primer registro.
- **De paso, tres cosas fuera de paleta** (`docs/principles.md`): el emoji 🛒 del botón de Sodimac, el aviso celeste de las reglas del banco, y los badges slate/índigo de Gastos.
- **Verificación.** El mismo auditor sobre las 24 pantallas: **0 se salen de ancho a 390px y 0 a 1440px**, 0 campos bajo 16px. En el barrido de escritorio cada pantalla muestra **exactamente una** tabla visible (las tarjetas no se duplican) y las mismas filas que antes. `tsc` limpio; lint **40 problemas antes y 40 después** — ni uno nuevo. **No toca `metrics.ts`, ni el schema, ni ningún cálculo.**

## 2026-08-04 — Configuración deja de estar desparramada y las pantallas de ajustes funcionan en el celular

- **Antes no existía una pantalla de Configuración.** Las dos pantallas de ajustes (Reembolsadores, Auditoría de precios) colgaban sueltas al final de un menú lateral plano de 12 ítems, y "Mi cuenta" estaba escondida detrás del nombre de usuario en el header. Ahora hay un hub en `/configuracion` que las junta, más las dos pantallas de reglas que viven dentro de Facturas y Banco — esas se **linkean, no se mudan**: sus rutas no cambian y siguen accesibles desde su sección de origen. Cada fila trae el dato en vivo (cuántas reglas, cuántas personas) para saber si vale la pena entrar.
- **Safari en iPhone hacía zoom en TODOS los campos de TODA la app, y no volvía solo.** La causa estaba en `globals.css`: `input, select, textarea { font-size: 0.75rem !important }` deja cada campo en 12px, y Safari hace zoom automático abajo de 16px. Ninguna clase por componente puede ganarle a ese `!important`. Se agregó un media query de `max-width: 767px` que sube los campos a 16px **solo en el celular** — en desktop la densidad de 12px queda intacta (verificado: 12px antes y después a 1440px). Los dos `:not()` de la regla no son decorativos: suben la especificidad lo justo para ganarle a `.text-sm`/`.text-base`, y respetan las utilidades `text-force-*` de las tablas densas, donde el tamaño chico es una decisión explícita.
- **Emojis fuera del menú lateral.** Los 12 ítems tenían un emoji cada uno (📊 📝 🏗️ 💵 🏦 🧾 📋 🧱 🚿 🔩 🔄 🔍), contra lo que dice `docs/principles.md`. Se reemplazaron por SVG monocromáticos en `ui/Glyph.tsx` (el repo no tiene `lucide-react`; se sigue el patrón inline que ya usaba `MovementResolveMenu.tsx`). El menú además quedó agrupado en tres bloques y con scroll propio: en un iPhone SE los últimos ítems quedaban fuera de pantalla sin forma de llegar.
- **Las dos pantallas de ajustes eran tablas anchas.** Reembolsadores (5 columnas con anchos fijos) y Auditoría de precios (4 columnas, dos de texto largo) ahora tienen una vista de tarjetas abajo de `md`/`sm`, con la misma información apilada. En desktop la tabla densa de siempre queda **igual, sin cambios**. En Reembolsadores el formulario de alta arranca cerrado en el celular: ocupaba la pantalla entera y tapaba la lista, que es a lo que se entra normalmente.
- **Un espacio que faltaba en la app en vivo.** En la bajada de Reembolsadores se leía "glosa**de** un reembolsador": JSX se comía el espacio después de `</span>`. Mismo caso en Auditoría de precios. Corregido con `{" "}` explícito y verificado en el HTML servido.
- **Verificación.** Capturas reales a 390px (iPhone) y 1440px de las tres pantallas, más un chequeo automático en pantalla: **ningún** campo abajo de 16px, **ningún** desborde horizontal y **ningún** botón abajo de 40px de alto táctil. `tsc` limpio; lint limpio salvo un error de `set-state-in-effect` en `Sidebar.tsx` que ya venía de `main`. **No toca `metrics.ts`, ni el schema, ni ningún cálculo.**

## 2026-08-03 — Capítulos del presupuesto y categorías del catálogo pasan a ser UN solo vocabulario

- **La causa: dos listas escritas a mano.** `CAPITULOS_SUGERIDOS` (lib/presupuesto/chapters.ts, 8 nombres, lo que ofrece el botón "+ Capítulo") y `CATALOG_CATEGORY_ORDER` (lib/utils.ts, 12 nombres, las categorías del Catálogo de Partidas). Nadie las mantuvo iguales y divergieron: el capítulo `INSTALACIONES ELECTRICAS` contra la categoría `INSTALACIONES ELECT.`, `INSTALACIONES SANITARIAS Y GASFITERIA` contra `INSTALACIONES SANITARIAS`, `LIMPIEZA Y ASEO` contra `ASEO Y LIMPIEZA`. Guardar una partida al catálogo tenía que adivinar a cuál hacerle caso.
- **Ahora hay una sola: `CAPITULOS`** en `lib/presupuesto/chapters.ts`. `CATALOG_CATEGORY_ORDER` se re-exporta desde ahí y `CAPITULOS_SUGERIDOS` se deriva (sin MUEBLES, que tiene su propio presupuesto; con ADICIONALES, que a propósito NO es categoría del catálogo — es un cajón por proyecto, no un tipo de trabajo).
- **Ganaron los nombres del presupuesto, y el motivo es el que manda:** el nombre del capítulo **sale en el PDF que ve el cliente**, y de los mal calzados hay 14, 17 y 17 en versiones ya enviadas o aprobadas — renombrarlos cambiaría documentos emitidos. Las categorías del catálogo las ve solo BLARQ. **No se tocó ni un capítulo de ningún presupuesto.**
- **Migración de la viva** (`scripts/homologar-categorias-catalogo.ts`, simulacro por defecto, exige `--aplicar --si-la-viva`, deja respaldo y aborta si detecta choques con `@@unique([category, name])`): `INSTALACIONES ELECT.` → `INSTALACIONES ELECTRICAS`, `INSTALACIONES SANITARIAS` + `INSTALACIONES GAS` → `INSTALACIONES SANITARIAS Y GASFITERIA`, `ASEO Y LIMPIEZA` → `LIMPIEZA Y ASEO`. Ninguna partida cambia de precio, unidad ni desglose: solo la etiqueta `category`. **El orden va DESPUÉS del deploy**, porque el código viejo no conoce los nombres nuevos y los mandaría al final de la pantalla.
- **De paso se reúne una categoría que estaba partida en dos.** En la viva convivían `ASEO Y LIMPIEZA` (6 partidas) y `LIMPIEZA Y ASEO` (3) — la segunda la había creado el guardado mudo que se sacó el mismo día, que escribía el nombre del capítulo tal cual. Era exactamente la basura que ese mecanismo generaba.
- **`homologarCategoria` cambia de papel**: era el puente entre dos listas, ahora es la RED — traduce los nombres viejos que quedan en datos históricos y aguanta que MJ escriba a mano con tildes o sinónimos. Además `compareCatalogCategories` resuelve los nombres ANTES de comparar, así un nombre viejo y su reemplazo caen en la MISMA posición mientras convivan (entre el deploy y la migración) y no aparece una categoría suelta al final.
- **Verificación.** `test-guardar-al-catalogo` 31/31 (incluye que viejo y nuevo ordenen igual, y que una categoría desconocida siga yendo al final) y `test-al-catalogo-perdida` en verde. La migración se aplicó primero completa contra dev y se re-corrió: 58 de 234 partidas movidas, 234 antes y 234 después, segunda corrida sin nada que mover. Antes/después en pantalla en `scripts/verify-homologar-categorias.ts`: 11 categorías antes y 11 después, mismos conteos, mismo orden, solo cambian 3 nombres.

## 2026-08-03 — Una partida armada de cero se puede guardar al catálogo como molde reusable

- **El agujero.** Los botones "a catálogo" del editor de presupuesto solo servían para partidas que **ya venían** del catálogo: `pushObraItemComponentsToCatalog` tira `SinCatalogoError` cuando el `ObraItem` no tiene `catalogPartidaId`. Una partida nueva, con todo su desglose de materiales y mano de obra, se moría en esa cotización — para reusarla había que rearmarla a mano en el Catálogo de Partidas.
- **El botón nuevo.** "↑ Guardar al catálogo" en el panel de la partida, visible **solo** cuando la partida no tiene molde. Crea la `PartidaCatalog` (nombre, unidad, descripciones, P.U. y los 6 rubros), engancha el `ObraItem` al molde, y copia el desglose reusando `pushObraItemComponentsToCatalog` — que ya sabe traducir el puntero de la pérdida a ids del catálogo. Endpoint `POST /api/presupuestos/[id]/partidas/[itemId]/crear-en-catalogo`; el `GET` del mismo path es el previo que decide si mostrar el botón.
- **Homologación capítulo → categoría** (`src/lib/catalog/homologarCategoria.ts`). Son dos vocabularios que nunca calzaron: el capítulo "INSTALACIONES ELECTRICAS" corresponde a la categoría "INSTALACIONES ELECT.", "INSTALACIONES SANITARIAS Y GASFITERIA" a "INSTALACIONES SANITARIAS", "LIMPIEZA Y ASEO" a "ASEO Y LIMPIEZA". Sin el puente, cada guardado creaba una categoría nueva casi idéntica que además cae al final del catálogo (`compareCatalogCategories` manda las desconocidas al fondo). Un capítulo sin equivalente (caso real: ADICIONALES) sí crea categoría nueva: es genuina, no un duplicado. **Esto no renombra ningún dato** — los capítulos del presupuesto siguen como MJ los escribió y el PDF del cliente no cambia.
- **Nunca duplica ni pisa.** `PartidaCatalog` tiene `@@unique([category, name])`. Si ya hay un molde con ese nombre en esa categoría (comparando **sin distinguir mayúsculas**, porque "Pintura muro" y "PINTURA MURO" son un duplicado para MJ), el botón no se ofrece: en su lugar aparece "Ya está en el catálogo: NOMBRE ↗" con link al molde existente. El server revalida al crear, porque el nombre de la partida se edita inline y puede cambiar con el panel abierto.
- **Se sacó el guardado mudo al crear.** Hasta ahora, crear una partida a mano mandaba en silencio al catálogo un molde **vacío** (nombre + unidad + precio), sin desglose —todavía no existe al crear— y sin quedar enganchado al `ObraItem`. Ensuciaba el catálogo sin servir de nada y chocaba por nombre con el guardado de verdad. El rótulo del formulario ("Se guardara automaticamente en el catalogo") decía eso y ya no era cierto.
- **Trampa cubierta.** Una partida **sin** desglose no dispara la copia de componentes: `pushObraItemComponentsToCatalog` termina llamando a `recalcPartidaTotals`, que recalcula el molde desde sus líneas y dejaría en $0 justo los montos globales recién copiados.
- **Alcance y verificación.** No toca `metrics.ts` ni el schema: crear un molde no mueve ningún total del presupuesto. `scripts/test-guardar-al-catalogo.ts` 24/24 (con desglose, sin desglose, nombre repetido, partida ya enganchada, homologación) y `test-al-catalogo-perdida` sigue en verde. Recorrido completo en pantalla contra dev en `scripts/verify-guardar-al-catalogo.ts`, con capturas antes/después; todo lo sembrado se borra en un `finally`.
...
- **De paso, un campo muerto menos.** `ArtefactoCatalog.clientPrice` existía desde junio con 53 valores guardados y **ningún flujo lo leía**: la pantalla, el margen, el alta en cotización y la bajada a borradores calculan todos lista × (1 − dcto). El comentario del schema afirmaba lo contrario (que `discountPercent` era "legacy"). Se sacó del código y del schema, con los valores respaldados; el `DROP COLUMN` va **después** del deploy (`scripts/sql/2026-07-31-drop-artefacto-catalog-clientprice.sql`), porque el código viejo en producción todavía selecciona la columna. No confundir con `ArtefactoItem.clientPrice`, que sí se usa y no se toca.
- **Alcance y verificación.** No toca `metrics.ts`. Verificado contra dev sembrando el caso real y borrándolo después: el WC pasa a "Distintos", muestra 30% → 39% y al aplicar queda en $139.990. `test-artefactos-propagacion` 10/10. Informe completo en [`auditoria-artefactos-precios-2026-07.md`](auditoria-artefactos-precios-2026-07.md); quedan pendientes la limpieza de los datos ya envenenados y el campo `clientPrice` del catálogo, que nadie lee.
...
- **Qué se agregó.** Dos columnas a `Invoice` — `montoObra` y `montoMuebles` — que junto a las 3 de artefactos permiten repartir la factura entre los **cinco** conceptos del Cuadro Resumen. Helpers `tieneDesgloseCobro` / `desgloseDeCobro` en `conceptoCobro.ts`: si CUALQUIERA de los cinco está seteado manda el desglose (los vacíos se leen como 0); si los cinco son null, todo se comporta exactamente como antes. Cada pago se prorratea por la fracción que representa del total de la factura — el mismo criterio que ya usaban los 3 de artefactos.
- **Dónde se usa.** `cuadroResumen.ts` (las columnas y el total pagado por concepto) y `fondoSueldos.ts` (cuánta plata se puede traspasar a sueldos, que crece con lo cobrado de obra y de muebles). El componente `DesgloseArtefactos` pasó a ser `DesgloseCobro`, con los 5 campos, un botón "completar" que calcula el monto que falta, y ahora se ofrece en **todas** las facturas emitidas, no solo las de concepto artefactos.
- **De paso, el quinto lugar de `noCobrado`.** `fondoSueldos.ts` también sumaba las partidas que BLARQ absorbe dentro del costo directo (los otros dos se arreglaron en el PR #346). El GG de Portofino baja de $8.310.056 a $8.125.056.
- **GOTCHA que se comió esta misma carga.** El script creó los `InvoicePayment` a mano y NO recalculó el estado de los movimientos: quedaron 6 movimientos diciendo "Pendiente" con su factura F-183 al lado. Desde el PR #333 el `status` de un `BankMovement` es DERIVADO (`recomputeMovementStatus` en `movementStatus.ts`) — **cualquier script que cree o borre imputaciones tiene que recalcular**, no alcanza con escribir el `InvoicePayment`. Reparado con `scripts/fix-portofino-estado-movimientos.ts` (los 24 movimientos de la clienta quedaron conciliados) y el script de carga corregido para no repetirlo.
- **Migración quirúrgica.** `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` sobre las dos bases (viva y desarrollo), aditivo y nullable: las 2.067 facturas de la viva quedaron intactas y sin desglose. **GOTCHA**: hay que reiniciar el dev server después de `prisma generate` o el cliente en memoria devuelve las columnas nuevas como `undefined` y el desglose se lee como cero (pasó en la verificación).
...
- **Dos hallazgos de plata.** (1) El Excel y el PDF que firmó la clienta **no coinciden**: el calefont (4.1) vale $468.393 en el Excel y **$343.393 en el PDF**, porque esa celda es un `SUMIF` vivo contra BASE DATOS que se movió después de emitir el PDF. Manda el PDF. (2) Los cuatro "descuentos" del Excel ($642.440) **sí eran al cliente** — el PDF los resta después del IVA — aunque la fórmula del propio Excel sumaba solo dos de los cuatro.
- **El descuento, sin soporte en la app.** `discountPercentage` solo se lee en muebles; en obra no lo mira nadie. Se representó como **partida negativa de −$425.091** en ADICIONALES: dividido por 1,27 × 1,19, al pasar por GG + utilidad + IVA vuelve a valer los $642.440 y el total cae exacto en **$61.396.984**, igual que el PDF. Su material y su MO quedan en 0 para no ensuciar el presupuesto por categoría.
- **El bug que destapó.** `ObraItem.noCobrado` existía desde el 2026-06 pero **ninguna partida de la base lo tenía marcado**, así que nunca se vio que solo lo respetaban `metrics.ts` y el PDF del cliente. `cuadroResumen.ts` y el resumen del `ObraEditor` sumaban las partidas absorbidas **dentro del total al cliente**: el Cuadro Resumen mostraba $62.794.936 y la tarjeta "Total Acordado" de la misma página $61.396.984, y el GG que se traspasa a sueldos salía inflado. Arreglado en los dos lugares; el editor ahora muestra el monto absorbido en su propia línea ("No cobrado (lo absorbe BLARQ)") para que la plata no desaparezca de la pantalla.
- **Alcance del arreglo.** De las **27 versiones de obra** de la base viva, **una sola** tiene partidas marcadas (Portofino V7). Las otras 26 no pueden moverse. Snapshot antes/después de 3 proyectos: solo se movió el #60, y `cobrado`, `gastado` y `utilidad real` quedaron idénticos (salen de facturas). Verificado con capturas de la app corriendo contra la base viva.
...
- **Agrupación (decisión MJ, tomada viendo las dos opciones renderizadas con datos reales).** Va **por ambiente**, en el orden que MJ arrastró en el editor y respetando los nombres libres de los bloques (ej. "TEKA"). Un WC que va 1 en cada uno de 3 baños sale en **3 líneas de cantidad 1**. Se descartó la alternativa de consolidar una línea por modelo con la cantidad total agrupada por marca (más columna "va en").
- **Cómo no se cuela un precio.** El módulo `src/lib/pdf/OrdenCompraArtefactosPDF.html.ts` es **self-contained** (mismo criterio que `MueblistaPDF`) y su tipo de entrada **ni siquiera tiene** `listPrice`/`clientPrice`/`realCostBlarq` — el PDF del cliente no se toca. Encima, `scripts/verify-oc-sin-precios.ts` verifica contra la base viva por dos vías: patrones de plata en el texto visible (signo $, montos con separador de miles, rótulos Dcto / P. lista / IVA) **y** que ningún monto real de cada ítem, formateado como lo escribe la app, aparezca en la hoja. **181 chequeos, 0 fallas** sobre Paseo del Sena V3. Ojo al escribir chequeos así: hay que descartar los `data:` URI, porque el logo en base64 contiene "IVA" y "Dcto" por azar.
- **Formato de la hoja.** Sale en **una sola hoja** de ancho A4 y alto variable, no en A4 paginado: se lee en pantalla y ahí partirla en páginas solo estorba (decisión MJ). Eso agrega la opción `singlePage` a `renderPDF.ts`, que es **compartido por los 10 PDFs de la app** — con la opción apagada el comportamiento es idéntico al anterior. Si alguien la reusa: el viewport hay que fijarlo al ancho imprimible **antes** del `setContent`, y el alto se mide **después** de esperar fuentes e imágenes, o la hoja corta el final. El logo del encabezado es el **isotipo** a 40px, no el wordmark de 31mm: esta hoja no tiene portada, así que es una página de detalle, y esa es la convención de obra/muebles/artefactos (`.dhead-iso`).
- **Alcance / plata.** **NO toca `metrics.ts`**, ni el schema, ni datos. Solo lectura: agrega una rama `tipo=orden-compra&sub=…` al route de PDF que ya existía y tres botones en la pantalla del presupuesto. Encabezado homologado con los PDFs al cliente (marca v2), no con el formato Montserrat que todavía usan el maestro y el mueblista.

## 2026-07-27 — Estado del movimiento bancario: fuente única + rótulo "Conciliado" (no más "Pagado")

- **Qué.** (1) El `status` de un movimiento del banco (`BankMovement`) deja de escribirse a mano: estaba repartido en **13 puntos de 8 archivos** (import de cartola, PATCH del movimiento, acciones masivas, reembolso de boletas, borrar pago desde la factura, compensar NC, auto-match, reglas de proveedor), con la cuenta "¿cuánto quedó imputado?" copiada 4 veces. Ahora todo pasa por `src/lib/banco/movementStatus.ts` (derivación pura + `recomputeMovementStatus`, espejo del `recomputeInvoiceStatus` del lado factura). (2) El rótulo visible del resuelto cambia: **"Pagado" → "Conciliado"** en la columna Estado y las pestañas de `/banco/movimientos` — "pagado" no calza para un ingreso, y "Conciliado" es el mismo verbo que ya usa toda la zona banco y las tarjetas de arriba (decisión MJ; se evaluó "Sin imputar / Imputado" y prefirió este vocabulario). Pendiente/Parcial quedan igual; los valores en la base y en la URL no cambian.
- **Bug tapado.** **Borrar una factura** dejaba al movimiento "Pagado" fantasma para siempre: los pagos se iban en cascada y nadie recalculaba el movimiento (el caso SODIMAC). Ahora el DELETE de facturas recomputa los movimientos que la pagaban (incluido el de reembolso si la factura era una NC). De paso: categorizar un movimiento con pagos ya no pisa su estado a `sin_factura`, y el desasignar masivo recomputa en vez de forzar `sin_asignar` parejo.
- **Modelo.** Dos familias de estados: derivados de la imputación (`conciliado`/`parcial` — la plata manda; sin nada imputado decide el reposo `sin_asignar`/`sin_factura`) y modos explícitos (`interno`/`neto_cero`) que el recompute nunca pisa. OJO: "tiene categoría" NO implica `sin_factura` — el import sugiere categorías en movimientos que nacen `sin_asignar`.
- **Alcance / plata.** **NO toca `metrics.ts`** ni el schema ni migra datos. Snapshots antes/después idénticos en dev: estados de los 654 movimientos, métricas de los 18 proyectos, EERR Caja 2025-2026. Test puro `scripts/test-movement-status.ts` 16/16; e2e con capturas `scripts/verify-estado-movimientos.ts` (nace → conciliar → desasignar → borrar factura, 5/5, datos de prueba limpiados).

## 2026-07-27 — Capítulos de obra: nombre libre y orden a mano

- **Qué.** Los capítulos del presupuesto de obra dejan de ser una lista fija de 8 nombres inamovibles (Demoliciones, Obra gruesa, …) y pasan a ser **filas propias de cada versión**, con **nombre que escribe MJ** y **posición que se arrastra** — el mismo modelo que ya tenían los capítulos de muebles (`MuebleChapter`). Se puede crear, renombrar, reordenar y borrar (borrar solo si está vacío). Un presupuesto nuevo arranca **sin capítulos**; el botón "+ Capítulo" ofrece los 8 de siempre como sugerencia de un click, más un campo libre.
- **Por qué.** No se podía llamar a un capítulo "TERRAZA Y JARDIN" ni poner Adicionales antes de Limpieza. Además el enum era frágil: una partida cuya clave no calzaba con ninguna de las 8 **desaparecía** del editor y del PDF pero seguía sumando en el total. Portofino V6 tenía 17 partidas así ($2.161.958): el cuadro de capítulos del PDF no cuadraba con su propio total.
- **Modelo.** Modelo nuevo `ObraChapter` (`budgetVersionId`, `name`, `sortOrder`) y `ObraItem.chapterId` con `ON DELETE SET NULL` (**no** Cascade: borrar un capítulo nunca puede llevarse partidas presupuestadas). El EP guarda su **propia foto** del capítulo (`EstadoPagoItem.chapterName` + `chapterOrder`), porque una partida `outOfScope` puede pertenecer a un capítulo que ya no existe. Las columnas viejas `ObraItem.chapter` y `EstadoPagoItem.chapter` **NO se borran**: quedan como red de seguridad hasta verificar en prod (lección del incidente #178).
- **Un solo nombre.** Antes había nombre corto de pantalla ("Sanitarias") y nombre formal de PDF ("INSTALACIONES SANITARIAS Y GASFITERIA"). Ahora es **uno solo**, y la migración carga el **formal** — así ningún PDF ya emitido cambia (decisión MJ 2026-07-27).
- **Fuente única.** La regla de agrupar, saltar capítulos vacíos y renumerar 1,2,3… vivía **copiada en seis lugares** (editor, PDF cliente, PDF maestro, Excel maestro, EP en pantalla, PDF del EP). Ahora vive en `src/lib/presupuesto/chapters.ts`. Ese helper además **rescata** las partidas sin capítulo en un capítulo sintético al final: una partida puede quedar mal ubicada, pero nunca invisible.
- **Alcance / plata.** **NO toca `metrics.ts`** — los totales nunca dependieron del capítulo. Regresión verificada partida por partida: **23 de 24** versiones de obra de la base viva producen un PDF **idéntico** (`scripts/verify-pdf-sin-cambios.ts --simular`); la única que cambia es Portofino V6, donde las 17 partidas escondidas pasan a verse y el descuadre desaparece. En dev, 11/11 chequeos de duplicar versión, comparador, resumen y página del maestro (`scripts/verify-otros-caminos.ts`).
- **EN PROD desde el 2026-07-27 (PR #331).** Migración de la viva hecha ANTES del deploy (las columnas nuevas son aditivas, el código viejo las ignoraba, así que la app no se cortó en ningún momento). SQL aditivo escrito a mano en `scripts/sql/capitulos-obra-viva.sql` (NO usar `prisma migrate diff`: mezcla cambios de otras ramas, hoy incluye un `DROP COLUMN "revisado"` ajeno con 350 valores en prod) + relleno con `scripts/migrar-capitulos-obra.ts` (dry-run y `--apply`). Resultado: **138 capítulos** sobre **935 partidas**, **0 sin capítulo**, conteos de filas idénticos antes/después. El script se puede volver a correr: recoge partidas que hayan quedado sueltas entre la migración y el deploy.
## 2026-07-27 — Conciliar es UN gesto: mismo verbo, mismo criterio, y el masivo propone antes de aplicar

- **Qué.** Los tres caminos para pegarle una factura a un movimiento del banco pasan a ser uno solo. (1) El menú Resolver dice **"Conciliar…"** (antes "Asignar a factura…"), el botón de arriba dice **"Conciliar pendientes (N)"** (antes "Auto-conciliar pendientes") y el modal se titula **"Conciliar movimiento"** (antes "Asignar pagos") — un verbo, no tres. (2) Arriba del modal aparece **siempre** la caja **Sugerencia**, con tres caras: *calza sola* (verde), *calza el monto pero mirala* (ámbar, con el motivo de la duda) y *no encontré ninguna* (gris). (3) El masivo pasa a **proponer → tildar → conciliar**: no escribe nada hasta que MJ confirma.
- **Por qué.** El nudo no eran solo tres botones: el masivo decidía con **RUT + monto** y la sugerencia del modal con **solo monto**, así que la app podía dejar un movimiento pendiente por desconfiar del RUT y, para ese mismo movimiento, mostrar un cartel verde que se veía confirmado (caso Cristóbal Rojas → Vidrios Rotos). Dos opiniones distintas del mismo dato. Ahora hay una sola: `proposeMovementInvoiceMatch` comparte con el auto-match tanto el universo de candidatas como el núcleo de decisión (`decideMovementInvoiceMatch`).
- **Las "probables" dejan de ser invisibles.** Las que el motor no ataba se quedaban pendientes en silencio. Ahora se muestran rotuladas y **destildadas**, con la frase de por qué se duda ("el monto calza al peso, pero el RUT no: la plata salió a X y la factura la emite Y"). En dev aparecieron 8 que nunca se habían visto.
- **Alcance / plata.** **NO toca `metrics.ts`** ni el schema ni ningún criterio de match. El camino que escribe se extrajo tal cual a `applyAutoMatchPayment` (mismas tres operaciones: pago `autoMatched`, movimiento a conciliado limpiando el rótulo pago-a-socio, recálculo de la factura) y lo usan por igual el auto-match del importador/sync y el masivo nuevo. Verificado contra dev: el conjunto de "seguras" es **idéntico** par por par a lo que ataba el motor viejo (`scripts/verify-conciliar-unificado.ts`); proponer no escribe nada (407 pagos antes y después); y aplicar+deshacer deja la base igual (`scripts/verify-conciliar-aplicar.ts`). Test puro `scripts/test-conciliacion.ts` 18/18.
- **Nada se perdió.** Auto-conciliar en lote, sugerencia y asignación manual siguen las tres. Deshacer sigue en Resolver ▾ → Desasignar (fila y lote); por eso se sacó el panel de "deshacer" post-corrida, que era el mismo botón en otro lugar.
