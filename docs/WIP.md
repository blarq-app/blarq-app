...
- **Ronda 40 — Bot de Telegram para asignar centro de costo desde una foto (implementado, sin desplegar)**:
  ...
  - **Lector de foto verificado con clave real (2026-05-31)**: MJ creó cuenta de API (Individual, USD 5 cargados, recarga automática OFF) y pasó la `ANTHROPIC_API_KEY` (guardada en `.env`). Probado: (a) llamada mínima al modelo `claude-haiku-4-5-20251001` → 200 OK (el nombre del modelo existe); (b) `readInvoicePhoto` contra una factura SODIMAC de prueba (SVG→PNG) → leyó RUT emisor 96792430-K (NO se confundió con el RUT de BLARQ receptor), folio 8845213, total 45990 (sin puntos), fecha 2026-05-28, nombre, confianza 0.98. El lector funciona de punta a punta. (Detalle local: dotenv NO pisa el env del shell — en local hay que correr `config({override:true})` para tests; en Vercel `process.env` viene directo, sin este problema.)
  - **PENDIENTE para desplegar**: (1) `ANTHROPIC_API_KEY` ✓ ya está en `.env` local — falta copiarla a Vercel. (2) IDs de Telegram de MJ y JT (cada uno manda `/start` al bot desplegado -> responde el ID -> a `TELEGRAM_ALLOWED_IDS`). (3) env vars en Vercel (BOT_TOKEN, WEBHOOK_SECRET, ALLOWED_IDS, ANTHROPIC_API_KEY). (4) `db push` a **prod** del modelo `PendingProjectTag` (confirmar con MJ). (5) correr `telegram-set-webhook.ts`. El ciclo completo NO se puede probar en local (Telegram necesita URL pública).
