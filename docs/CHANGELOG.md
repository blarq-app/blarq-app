...
## 2026-07-27 — Estado del movimiento bancario: fuente única + rótulo "Sin imputar / Imputado"

- **Qué.** (1) El `status` de un movimiento del banco (`BankMovement`) deja de escribirse a mano: estaba repartido en **13 puntos de 8 archivos** (import de cartola, PATCH del movimiento, acciones masivas, reembolso de boletas, borrar pago desde la factura, compensar NC, auto-match, reglas de proveedor), con la cuenta "¿cuánto quedó imputado?" copiada 4 veces. Ahora todo pasa por `src/lib/banco/movementStatus.ts` (derivación pura + `recomputeMovementStatus`, espejo del `recomputeInvoiceStatus` del lado factura). (2) El rótulo visible cambia: **Pendiente/Parcial/Pagado → "Sin imputar" / "Parcial" / "Imputado"** (columna Estado, pestañas de filtro y tarjetas de `/banco/movimientos`) — "pagado/pendiente" no calza para un ingreso; el eje real es la imputación. Los valores en la base y en la URL no cambian.
