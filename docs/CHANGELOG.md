...
- **Botón "Revisar precios"**: `POST /api/catalogo/artefactos/revisar-precios` scrapea el precio web de hoy (reusa `fetchArtefactoData` — mk.cl/sodimac/easy) y muestra guardado vs web ahora; MJ aplica los que quiere (uno o todos). NO pisa nada solo: al aplicar sube el precio lista; el dcto y el costo no se tocan. Detección automática del descuento del web = best-effort (depende de qué publique cada tienda).
