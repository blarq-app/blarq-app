...
- **Ronda 34 — Limpieza de conciliaciones mal asignadas + criterio conservador (sesión con MJ)**:
  ...
  - **Bug detectado, NO arreglado**: `src/app/api/banco/auto-conciliar-pendientes/route.ts` tiene OTRA copia inline del auto-match con el mismo bug "1 candidato sin chequear RUT" que tenía el importador. Conviene unificarla con `tryAutoMatchMovementWithInvoices` (igual que se hizo con el import). Pendiente.
