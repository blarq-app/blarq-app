...
- **Ronda 40 — Bot de Telegram para asignar centro de costo desde una foto (implementado, sin desplegar)**:
  ...
  - **Estado de deploy (2026-05-31)**: código mergeado a `main` y pusheado (commit `f4302be`) → Vercel desplegando. Tabla `PendingProjectTag` creada en **prod** (backup previo `backups/blarq-prod-2026-05-31T23-23.json.gz`; tabla vacía, 0 filas, data existente intacta). PENDIENTE: (1) env vars en Vercel (BOT_TOKEN, WEBHOOK_SECRET, ALLOWED_IDS, ANTHROPIC_API_KEY) — sin esto el deploy levanta pero el bot no funciona. (2) IDs de Telegram de MJ y JT (cada uno manda `/start` al bot ya desplegado -> el bot responde el ID -> a `TELEGRAM_ALLOWED_IDS`). (3) correr `telegram-set-webhook.ts`. El ciclo completo NO se puede probar en local (Telegram necesita URL pública).
...
- **Ronda 39 — Research (sin código): boletas de honorarios del SII + importación automática del banco**:
  ...
  - **BHE 2026 — pendiente**: NO están en Maxxa (export es 2025). Fuente = SII directo (portal Boletas de Honorarios → boletas recibidas de terceros → exportar Excel → importar a la app). Próximo paso elegido por MJ: explorar el portal SII juntos (con MJ logueada al SII) para confirmar ruta/formato y armar el importador (sirve 2025+2026+adelante).
